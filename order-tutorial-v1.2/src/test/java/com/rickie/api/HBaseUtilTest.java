...
class HBaseUtilTest {
    ...
    void createTableTest() throws IOException {
        ...
        Boolean result = HBaseUtil.createTable("orders:orders" + format.format(calendar.getTime()), "info", "items");
    ...
    void deleteTableTest() throws IOException {
        ...
        Boolean result = HBaseUtil.deleteTable("orders:orders"+format.format(calendar.getTime()));
        Assert.assertTrue(result);
    }
    ...
    void putRowsTest() {
        ...
        for (int i = 0; i < 5; i++) {
            long orderId = random.nextInt(10000000);
    ...
    public void getRowsTest() {
        // 替换不同的rowkey
        List<String> rowkeys = Arrays.asList("36630000000000000000", "38250000000000000000");
...
//            });
    ...
    public void filterTableTest() {
        ...
        SingleColumnValueFilter orderIdFilter = new SingleColumnValueFilter(
                CF_INFO.getBytes(), "order_id".getBytes(),
                CompareFilter.CompareOp.GREATER_OR_EQUAL,"5000".getBytes());
        filterList.addFilter(orderIdFilter);
        ResultScanner scanner = HBaseUtil.filterTable(TABLE, filterList);
        scanner.iterator().forEachRemaining(result -> System.out.println(
                "=>" + new String(result.getRow()) +
                        " order_id:" + new String(result.getValue(CF_INFO.getBytes(), "order_id".getBytes()))));
    }
